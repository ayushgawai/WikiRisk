...
## One-line summary
WikiRisk is a backend-first, real-time Wikipedia edit risk detection system that uses Spark for batch and streaming processing, a saved SparkML model for scoring, FastAPI for serving, SQLite as the demo prediction store, MLflow for tracking, and OpenAI for optional explanation text.

## Current product state
- The project is already trained and runs locally.
- The frontend was removed; do not describe or rely on a Streamlit UI.
- The live demo is backend-only and centers on the API, the streaming processor, the SQLite store, and MLflow.
...
7. FastAPI serves `/health`, `/stats`, `/edits/recent`, `/edits/{id}`, and `/explain`.
...
- Keep both aligned with the backend-only stack and the current API / MLflow / SQLite flow.

## What another AI should not do
- Do not reintroduce Streamlit or describe it as part of the current demo.
...
The project should be presented as a production-style local demo with a credible big-data workflow: historical training, streaming inference, server-side persistence, and operator-friendly explanation output. The value is in the end-to-end integration, not in retraining during the presentation.
